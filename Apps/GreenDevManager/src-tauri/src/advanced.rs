...
pub(super) fn prepare_app_update(version: String) -> Result<OperationResult, String> {
    ...
    let pending = json!({"schemaVersion":1,"version":version,"preparedAt":now_millis(),"package":display_path(&portable.path()),"stage":display_path(&stage),"currentExecutable":display_path(&root.join(r"Apps\GreenDevManager\GreenDevManager.exe")),"state":"verified"});
    write_json_backup(
        &root,
