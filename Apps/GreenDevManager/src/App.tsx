...
function App() {
  ...
  const [appUpdate, setAppUpdate] = useState<AppUpdateStatus | null>(isDesktop ? null : { currentVersion: "1.1.0", latestLocalVersion: "1.1.0", latestRemoteVersion: "", targetVersion: "1.1.0", updateAvailable: false, prepared: false, localVersions: ["1.1.0"], settings: { schemaVersion: 1, channel: "stable", feedUrl: "https://github.com/MetaUoa/GreenDevManager/releases/latest/download/update-feed.json", requireSignature: false, autoDownload: false }, feed: {} });
  ...
  const [bootstrap, setBootstrap] = useState<BootstrapStatus | null>(isDesktop ? null : { configured: true, root: mockDashboard.root, currentVersion: "1.1.0", manifestUrl: "" });
...
function ManifestView({ items, updates, taskActive, onAction, onImport, onSettings, onBatch, onRollback, onRefreshCatalog, onAdopt }: { items: ManifestComponent[]; updates: UpdateCandidate[]; taskActive: boolean; onAction: (item: ManifestComponent, action: "install" | "update") => void; onImport: (item: ManifestComponent) => void; onSettings: () => void; onBatch: (ids: string[]) => void; onRollback: (id: string) => void; onRefreshCatalog: () => void; onAdopt: (id: string, candidateId: string | null) => void }) {
  ...
  return <div className="page-stack">
    <div className="notice"><PackageCheck size={18} /><div><strong>多来源绿色安装中心</strong><p>联网刷新后可选择发行版、LTS 版本和架构；采用候选后执行 SHA256 预检、依赖排序与失败回退，旧版本始终保留。</p></div><div className="button-row"><button className="secondary-button" disabled={taskActive} onClick={onRefreshCatalog}><RefreshCw size={15} />联网刷新</button><button className="secondary-button" onClick={onSettings}><Settings2 size={15} />镜像与代理</button></div></div>
    <section className="settings-strip"><div><strong>已选择 {selected.size} 个更新</strong><span>执行顺序由组件依赖自动计算。</span></div><button className="primary-button" disabled={!selected.size || taskActive} onClick={() => onBatch([...selected])}><Layers3 size={15} />批量更新计划</button></section>
    <section className="manifest-grid">{items.map(item => {
      const update = updates.find(value => value.componentId === item.id);
      const candidate = update?.candidates.find(value => value.id === selectedCandidates[item.id]) ?? update?.candidates[0];
      const displayName = item.id === "java" && new Set(update?.candidates.map(value => value.provider)).size > 1 ? "Java JDK" : item.name;
      return <article className={selected.has(item.id) ? "manifest-card selected" : "manifest-card"} key={item.id}>
        <div className="component-top"><label className="manifest-select"><input type="checkbox" checked={selected.has(item.id)} disabled={!update?.updateAvailable || update?.canAdopt || item.pinnedElsewhere} onChange={() => toggle(item.id)} /><ComponentGlyph id={item.id} /></label><span className={`phase-state ${item.state}`}>{update?.canAdopt ? `${update.candidates.length} 个候选` : stateLabel[item.state]}</span></div>
        <h3>{displayName}</h3>
        {update && update.candidates.length > 1 && <label className="source-picker"><span>绿色发行版 / 版本</span><select aria-label={`${displayName} 绿色发行版与版本`} value={candidate?.id ?? ""} onChange={event => setSelectedCandidates(current => ({ ...current, [item.id]: event.target.value }))}>{update.candidates.map(option => <option value={option.id} key={option.id}>{option.provider} · {option.version} · {option.architecture}</option>)}</select></label>}
        <div className="manifest-version"><span>{update?.currentVersion ?? "当前"} →</span><strong>{candidate?.version ?? update?.targetVersion ?? item.desiredVersion}</strong></div>
        <dl><dt>来源</dt><dd>{candidate ? `${candidate.provider} · ${candidate.channel}` : "本地清单"}</dd><dt>架构</dt><dd>{candidate?.architecture || "跟随清单"}</dd><dt>校验</dt><dd>{candidate ? candidate.checksumReady ? "SHA256 已锁定" : "需离线导入" : item.checksumReady ? "SHA256 已锁定" : "等待离线导入"}</dd><dt>依赖</dt><dd>{item.dependencies.join("、") || "无"}</dd></dl>
        <p className="release-note">{candidate?.notes ?? update?.releaseNotes}</p>
        {item.blockedReason && !update?.canAdopt && <div className="card-warning">{item.blockedReason}</div>}
        <div className="card-actions three"><button className="secondary-button" disabled={taskActive} onClick={() => onImport(item)}><Upload size={14} />导入</button><button className="secondary-button" disabled={taskActive || !item.active} onClick={() => onRollback(item.id)}><RotateCcw size={14} />回退</button>{update?.canAdopt ? <button className="primary-button" disabled={taskActive || item.pinnedElsewhere || !candidate} onClick={() => onAdopt(item.id, candidate?.id ?? null)}><Check size={14} />采用</button> : <button className="primary-button" disabled={!item.enabled || taskActive || item.active || item.pinnedElsewhere} onClick={() => onAction(item, item.installed ? "update" : "install")}><Download size={14} />{item.active ? "当前" : "计划"}</button>}</div>
      </article>;
    })}</section>
  </div>;
}
